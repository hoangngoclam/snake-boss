...
export default function ProfilePage(props: any) {
    ...
    useEffect(() => {
        try {
            const currentUser: any = JSON.parse(localStorage.getItem('user') || 'null');
            API.Get('http://localhost:5000/user/' + currentUser.id).then((result: any) => {
                const dataResult = result.data.user as IUserModel;
    ...
    const onButtonSubmitClick = async () => {
        const user: IUserModel = (userInfo as unknown) as IUserModel;
        ...
        const result: any = await API.PostFormData('http://localhost:5000/user/edit', bodyFormData).catch((e) => {
            setError('Save profile error');
        });
        const dataResult = result.data as ISaveProfileData;
    ...
    const onAvatarChange = async (_event: any) => {
        try {
            ...
            const data: any = await toBase64(files[0]);
            setImageUrl(data);
    ...
    if (userInfo === undefined) {
        ...
        const user: IUserModel = userInfo as IUserModel;
        return (
                    ...
                    <div className=" w-full lg:w-2/6 md:w-2/6 sm:w-4/6 mx-auto items-center flex justify-center mt-2">
                        <div className="py-5 px-12 border border-gray-400 w-full rounded-md">
                            ...
                            <InputComponent
                                ...
                                placeholder="Name"
                                value={displayName}
                                onChangeEvent={onDisplayNameChange}
                            />
